...
(function() {
  ...
  function createDrawer() {
      ...
      <div class="next-drawer-panel" id="nextAiDrawerPanel">
        <div class="next-drawer-header">
          ...
          <div style="display:flex;align-items:center;gap:8px">
            <button class="drawer-btn" id="nextDrawerResetQuotaBtn" title="Reset API Call Quota" style="color:#38bdf8;border-color:#0284c7">↻ Reset Quota</button>
        ...
        <div class="next-inspector-modal" role="dialog" aria-modal="true">
          <div class="inspector-header">
            <div>
              ...
              <div id="inspectorMeta" style="font-size:11px;color:#94a3b8;margin-top:2px;font-family:'IBM Plex Mono'">gemini-3.7-flash · Latency: 0.8s</div>
            </div>
    ...
    const resetQuotaBtn = document.getElementById('nextDrawerResetQuotaBtn');
      ...
      list.innerHTML = logs.map(l => {
        ...
        return `
          <div class="telemetry-row" onclick="window.nextOpenInspectorById('${l.id}')">
            ...
            <span style="color:#94a3b8;width:110px">${escapeHtml(l.model || 'gemini-3.7-flash')}</span>
  ...
  function openInspector(log) {
    ...
    title.innerText = `${log.title || log.promptName || 'Operation'} — Reasoning & Trace`;
    meta.innerText = `${log.model || 'gemini-3.7-flash'} · Latency: ${log.durationSec || (log.durationMs ? (log.durationMs/1000).toFixed(1)+'s' : '—')} · Citations: ${log.citationsCount || 0}`;
  ...
  window.nextFetchAiLogs = fetchAiLogs;
